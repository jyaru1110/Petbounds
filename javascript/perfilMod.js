function selecUsu() {
    ...
    fileSelector.addEventListener('change', (event) => {
        ...
        var enlace = 'http://161.35.132.152/api/foto?nom=' + fileList[0].name + '&cont=' + fileList[0].type;
        $.ajax({
            type: 'GET',
            url: enlace,
            success: response => {
    ...
    var enlace = 'http://161.35.132.152/api/mostrarUsu?correo=' + correo;
    $.ajax({
        type: 'GET',
        url: enlace,
        success: response => {
...
function modUsu() {
    ...
    var url = Cookies.get('foto');
    var enlace = 'http://161.35.132.152/api/modUsu?id=' + id + '&nom=' + nombre + '&pat=null&mat=null&nick=' + nick + '&nac=' + nacimiento + '&foto=' + url;
    $.ajax({
        type: 'PUT',
        url: enlace,
        success: response => {
...
function borrarUsu() {
    var correo = Cookies.get('correo');
    var enlace = 'http://161.35.132.152/api/borrarUsu?correo=' + correo;
    $.ajax({
        type: 'DELETE',
        url: enlace,
        success: response => {
